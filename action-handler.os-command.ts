...
export class OsCommandActionHandler implements ActionHandler {
    ...
    async execute(
        ...
        if (!instanceSettings.osCommand) {
            return createErrorResult("OS Command Action Error: Command not specified.", { settings: instanceSettings });
        }
        ...
        try {
            ...
            const commandPayload = { command: instanceSettings.osCommand, target: instanceSettings.osTarget };
            ...
            return createErrorResult(`OS command failed: ${message}`, { error, host: targetHost, settingsUsed: instanceSettings });
        } finally {
